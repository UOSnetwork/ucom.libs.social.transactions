...
class TransactionFactory {
   ...
   * @param {string} senderAccountNameFor
   * @param {string} blockchainId
   * @return {Promise<string>}
   */
  static async getSignedDirectPostCreationForUser(
    senderAccountName,
    senderPrivateKey,
    senderAccountNameFor,
    blockchainId) {

    const actionName = ActionNameDictionary.getDirectPostCreationForUser();
    const contentTypeId = ContentTypeDictionary.getTypeDirectPost();

    const data = {
      acc:              senderAccountName,
      acc_to:           senderAccountNameFor,
      content_id:       blockchainId,
      content_type_id:  contentTypeId
    };

    return this._getSignedTransaction(senderAccountName, senderPrivateKey, actionName, data);
  }
   ...
   * @param {string} senderAccountName
   * @param {string} senderPrivateKey
   * @param {string} organisationIdTo
   * @param {string} blockchainId
   * @return {Promise<string>}
   */
  static async getSignedDirectPostCreationForOrg(
    senderAccountName,
    senderPrivateKey,
    organisationIdTo,
    blockchainId
    ...
    const actionName = ActionNameDictionary.getDirectPostCreationForOrg();
    const contentTypeId = ContentTypeDictionary.getTypeDirectPost();

    const data = {
      acc:                senderAccountName,
      organisation_to_id: organisationIdTo,
      content_id:         blockchainId,
      content_type_id:    contentTypeId
    };
   ...
   * @param {string} senderAccountName
   * @param {string} senderPrivateKey
   * @param {string} blockchainId - new repost blockchain ID
   * @param {string} parentBlockchainId - parent post blockchain ID (which is reposted by user)
   * @return {Promise<string|Object>}
   */
  static async getSignedUserCreatesRepostOtherPost(
    senderAccountName,
    senderPrivateKey,
    blockchainId,
    parentBlockchainId
    ...
    const contentTypeId = ContentTypeDictionary.getTypeRepost();
   ...
   * @return {Promise<string>}
   */
  static async getSignedUserUpvotesContent(senderAccountName, senderActivePrivateKey, contentBlockchainId) {
    const interactionTypeId = InteractionTypesDictionary.getUpvoteId();

    return this.getSignedUserToContentActivity(senderAccountName, senderActivePrivateKey, contentBlockchainId, interactionTypeId);
  }
   ...
   * @return {Promise<string>}
   */
  static async getSignedUserDownvotesContent(senderAccountName, senderActivePrivateKey, contentBlockchainId) {
    const interactionTypeId = InteractionTypesDictionary.getDownvoteId();
   ...
   */
  static async getSignedUserToContentActivity(senderAccountName, senderActivePrivateKey, contentBlockchainId, interactionTypeId) {
    ...
    return this._getSignedTransaction(senderAccountName, senderActivePrivateKey, actionName, data);
  }
